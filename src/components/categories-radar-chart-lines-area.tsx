  ...
  return data
    .map((d, i) => {
      ...
      columns.forEach(({key, angle}) => {
        ...
        circles.push(<circle cx={x2} cy={y2} r={5} fill={color(`${i}`)} />);
      });

      return (
        <g key={`shape-${JSON.stringify(d)}`} transform="translate(0,0)">
          {area}
          {lines}
          {circles}
        </g>
    ...
    .reduce((memo, lines) => memo.concat(lines), [] as ReactElement[]);
};
